...
const youthjob_jobs_filtering = async (filters, searchphrase) => {
    ...
    filters.forEach(obj => {
        if (obj.marked){  
            ...
            objCopy.name == "Ort" ? where.location = {city: {_eq: objCopy.value}} : null
            objCopy.name == "Anställningstyp" ?  where.employment_type_name = {_eq: objCopy.value} : null
            objCopy.name == "Språk" ?  where.language_name = {_eq: objCopy.value} : null
            objCopy.name == "Företag" ? where.company = {name: {_eq: objCopy.value}} : null
        }
    ...
    if (searchphrase === ""){
        ...
        const res = await axios.post(
            url, 
            {query: query_with_searchphrase, variables: {where: where, searchphrase: searchphrase}}, 
            headers
        )
...
export default youthjob_jobs_filtering;