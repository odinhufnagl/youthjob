...
const Lst_info = ({ curUserInfo, email, refresh, showCompanyModal }) => {
    ...
    const lst = [["Email", email], ["Telefonnummer", phone_number]]
    ...
    return (
        <View style={{width: "90%", marginTop: 20}}>  
            ...
            <Lst_info_obj data={["Bio", bio]} refresh={refresh} bio={bio}></Lst_info_obj>

            <CV_and_personal_letter></CV_and_personal_letter>

            <Following following={following} countFollowing={following_aggregate.aggregate.count} showCompanyModal={showCompanyModal}></Following>

            <FlatList
            data={lst}
            keyExtractor={(item, index) => 'key:'+index}
            renderItem={({item}) => <Lst_info_obj data={item} refresh={refresh}></Lst_info_obj>}
            ></FlatList>
            <View style={{height: 30}}></View>
            <View style={{width: "90%", marginTop: 0, alignSelf: "center"}}>

            {["Visa Pågående utbildning", "Visa Senast avklarade utbildning", "Visa Tidigare jobb"].map((title) => 
            <TouchableOpacity onPress={() => {navigation.navigate("Edit", {title: title.split(' ').slice(1).join(' '), refresh: refresh})}} style={{width: 270}}>

                <Text style={{fontFamily: fonts[700], color: "rgba(0, 0, 0, 0.8)", fontSize: 15, marginTop: 16}}>
                   {title}
                </Text>
            </TouchableOpacity>
           )}
